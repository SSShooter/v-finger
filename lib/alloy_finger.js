...
AlloyFinger.prototype = {
  start: function (evt) {
    ...
    this.notSingleTap = false
    this.longTapTimeout = setTimeout(
      function () {
        this.longTap.dispatch(evt, this.element)
        this._preventTap = true
        this.notSingleTap = true
      }.bind(this),
      750
    )
  },
  move: function (evt) {
    ...
    //swipe
    if (
      (this.x2 && Math.abs(this.x1 - this.x2) > 120) ||
      (this.y2 && Math.abs(this.y1 - this.y2) > 120)
    ) {
      // 起始结束相差180判定为swipe
      ...
      !this.x2 ||
      (Math.abs(this.x1 - this.x2) < 5 && Math.abs(this.y1 - this.y2) < 5)
    ) {
      // 不足180且大于5判定
      ...
      // 250ms后判定为单击
      if (!self.isDoubleTap && !self.notSingleTap) {
